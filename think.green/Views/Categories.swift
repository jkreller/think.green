            ...
            BaseView {
                GeometryReader { geometry in
                    ScrollView(showsIndicators: false) {
                        GridStack(rows: (self.categoriesViewModel.thoughtCategories.count - 1) / 2 + 1, columns: 2, hAlignment: .leading) { index, row, col in
                            if (self.categoriesViewModel.thoughtCategories.endIndex > index) {
                                // Don't show last element if it's an odd amount of elements
                                CategoryBadge(
                                    category: self.categoriesViewModel.thoughtCategories[index],
                                    parentWidth: geometry.size.width,
                                    onTap: {
                                        self.chooseCategory(category: self.categoriesViewModel.thoughtCategories[index])
                                    }
                                )
                            }
                        }
